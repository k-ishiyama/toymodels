...
struct InputEventForImGui
{
	...
	void OnMouseWheel(const wnd::InputArgument& arg)
	{
		mImGuiIO.MouseWheel += arg.index > 0 ? +1.0f : -1.0f;
	}
	...
	{
		if(arg.index < 256)
			mImGuiIO.KeysDown[arg.index] = 1;
	}

	void OnKeyUp(const wnd::InputArgument& arg)
	{
		if (arg.index < 256)
			mImGuiIO.KeysDown[arg.index] = 0;
	}

	void OnChar(const wnd::InputArgument& arg)
	{
		if (arg.index > 0 && arg.index < 0x10000)
			mImGuiIO.AddInputCharacter(static_cast<unsigned short>(arg.index));
	}
